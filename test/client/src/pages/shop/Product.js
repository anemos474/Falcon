...
import { Field, AddToCartFormProvider, SEO, useSidebarContainer } from '@deity/falcon-front-kit';
import {
  ProductLayout,
  ProductLayoutArea,
  ProductDescription,
      ...
      <ProductQuery variables={{ ...params }}>
        {({ data: { product } }) => (
          <>
            ...
            <AddToCartFormProvider quantity={1} product={product} onSuccess={() => sidebar.open(SIDEBAR_TYPE.cart)}>
              {({ isSubmitting, status }) => (
